...
const DYNAMIC_BY_NEXTPAGE = `select message.*
                             from message inner join module on (message.moduleid = module.id)
                             __WHERE__
                             order by message.createdat desc
                             limit $(size^)`;

const DYNAMIC_BY_PREVPAGE = `with previous_page as (
                                select message.*
                                from message inner join module on (message.moduleid = module.id)
                                __WHERE__
...
const ALL_PUBLISHED_BY_CATID_AND_NPAGE = `select m.*
                                          from message m inner join module md on (m.moduleid = md.id)
                                          where m.createdat < $(createdat)
                                              and m.published = true
                                              and (m.moduleid = $(moduleid) or md.code = 'BLOG')
...
const ALL_PUBLISHED_BY_CATID_AND_PPAGE = `with previous_page as (
                                              select m.*
                                              from message m inner join module md on (m.moduleid = md.id)
                                              where m.createdat > $(createdat)
                                                  and m.published = true
...
module.exports = {
    ...
    DYNAMIC_BY_NEXTPAGE: DYNAMIC_BY_NEXTPAGE,
    DYNAMIC_BY_PREVPAGE: DYNAMIC_BY_PREVPAGE,
